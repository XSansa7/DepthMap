...
cmd:option('-duplicate', 12, 'number of duplicate')
...
if config.localt == false then
    train_image_path = '/scratch/' .. owener .. '/project/NewTrainImage/'
    train_depth_path = '/scratch/' .. owener .. '/project/NweTrainDepth/'
    train_split = '/home/' .. owener .. '/project/trainsplit.t7'
    test_split = '/home/' .. owener .. '/project/testsplit.t7'
    N = 1449
...
function rand_crop(input, target)
    ...
    --print(input:size())
    if kind == 1 then
        input = image.crop(input, "c", 218, 294)
        target = image.crop(target, "c", 218, 294)
    elseif kind == 2 then
        input = image.crop(input, "tl", 218, 294)
        target = image.crop(target, "tl", 218, 294)
    elseif kind == 3 then
        input = image.crop(input, "tr", 218, 294)
        target = image.crop(target, "tr", 218, 294)
    elseif kind == 4 then
        input = image.crop(input, "bl", 218, 294)
        target = image.crop(target, "bl", 218, 294)
    else 
        input = image.crop(input, "br", 218, 294)
        target = image.crop(target, "br", 218, 294)
    end
...
function getImage(idx)
    print(idx)
    local i1 = torch.floor(idx / duplicate)
    local i2 = idx - i1 * duplicate
    if i2 == 0 then
        i2 = duplicate 
    end
...
function getLabel(idx)
    ...
    return torch.load(train_label_path .. file):reshape(320*240,1):float()
end

function getDepth(idx)
    -- print(idx)
    local i1 = torch.floor(idx / 4)
    local i2 = idx - i1 * 4
    if i2 == 0 then
        i2 = 4 
    end
    local file = string.format("%04d", i1) .. string.format("-%04d.t7", i2)
    local depth = torch.load(train_depth_path .. file):reshape(1,320,240):float()
...
function BuildNetwork3()
    ...
    network3:add(nn.SpatialUpSamplingNearest(2)) -- 320 -1 / 5 + 1 = 128 96
    network3:add(nn.SpatialBatchNormalization(1,1e-3))
...
function BuildCoarse()
    ...
    -- 1
    network:add(nn.SpatialConvolution(3,96,11,11,4,4,1,1))
    network:add(nn.SpatialBatchNormalization(96,1e-3))
    network:add(nn.PReLU())
    network:add(nn.SpatialMaxPooling(2,2,2,2))
    network:add(nn.SpatialDropout(0.3))
    --network2:add(nn.SpatialDropout(0.3))
    --96 37 27

    --2
    network:add(nn.SpatialConvolution(96,256,5,5,1,1,1,1))
    network:add(nn.SpatialBatchNormalization(256,1e-3))
    network:add(nn.PReLU())
    network:add(nn.SpatialMaxPooling(2,2,2,2,1,1))
    network:add(nn.SpatialDropout(0.3))
    --256 18 13

    --3
    network:add(nn.SpatialConvolution(256,384,3,3,1,1,1,1)) -- 384 18 13
    network:add(nn.SpatialBatchNormalization(384,1e-3))
    network:add(nn.PReLU())
    --4
    network:add(nn.SpatialConvolution(384,384,3,3,1,1,1,1)) -- 384 18 13
    network:add(nn.SpatialBatchNormalization(384,1e-3))
    network:add(nn.PReLU())
    --5
    network:add(nn.SpatialConvolution(384,256,3,3,2,2,0,0)) -- 256 8 6
    network:add(nn.SpatialBatchNormalization(256,1e-3))
    network:add(nn.PReLU())
    
    --6
    network:add(nn.Reshape(256*9*6))
    network:add(nn.Linear(256*9*6, 4096)) -- 4096 1 1
    network:add(nn.BatchNormalization(4096))
    network:add(nn.PReLU())
    network:add(nn.Dropout(0.5))

    --7
    network:add(nn.Linear(4096, 74*55))
    network:add(nn.Reshape(1, 74, 55)) -- 1 74 55

    return network
end

function BuildPreFine()
    local network = nn.Sequential()
    network:add(nn.SpatialConvolution(3,63,9,9,2,2,1,1))
    network:add(nn.SpatialBatchNormalization(63,1e-3))
    network:add(nn.PReLU())
    network:add(nn.SpatialMaxPooling(2,2,2,2))
    network:add(nn.SpatialSubSampling(63, 4,5, 1,1))
    return network
end
...
function BuildFine()
    local network = nn.Sequential()
    -- 2
    network:add(nn.SpatialConvolution(64,64,5,5,1,1,2,2))
    network:add(nn.SpatialBatchNormalization(64,1e-3))
    network:add(nn.PReLU())
    --64 74 55
    ...
    -- 4
    network:add(nn.SpatialConvolution(64,1,5,5,1,1,2,2))
    network:add(nn.SpatialBatchNormalization(1,1e-3))
    network:add(nn.PReLU())
    --1 74 55

    network:add(nn.SpatialUpSamplingNearest(9))
    network:add(nn.SpatialBatchNormalization(1,1e-3))
    network:add(nn.SpatialSubSampling(1,16,27,1,1))
    network:add(nn.PReLU())
...
network1 = nn.Sequential()
network1:add(BuildCoarse())

network2 = nn.Concat(2)
network2:add(BuildPreFine())
network2:add(network1)

network = nn.Sequential()
network:add(network2)
network:add(BuildFine())
