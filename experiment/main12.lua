...
-- refer: https://github.com/albanD/gtsrb/blob/master/dataset.lua
...
function DepthCriterion:__init(sizeAverage)
   parent.__init(self)
...
local xfilter = torch.FloatTensor(1,3); xfilter[{1,1}] = -1; xfilter[{1,2}] = 0; xfilter[{1,3}] = 1; 
local yfilter = torch.FloatTensor(3,1); yfilter[{1,1}] = -1; yfilter[{2,1}] = 0; yfilter[{3,1}] = 1;
...
function DepthCriterion:updateOutput(input, target)
  ...
  local sum = 0;
  local diff = torch.csub(input, target) -- di 5,1,640,480
  print(diff:size())
  local product1 = torch.cmul(diff, diff) -- di^2 5,1,640,480
  product1 = torch.sum(product1:reshape(input:size(1), 640*480), 2) -- sum(di^2) --5,1
  local product2 = torch.sum(diff:reshape(input:size(1), 640*480), 2) -- sum of di -- 5,1
  product2 = torch.cmul(product2, product2) -- (sum of di )^2 5,1

  for i in 1, input:size(1) do
      local gradientx = torch.convolve(diff[{i,1,}], xfilter, 'same')
      local gradienty = torch.convolve(diff, yfilter, 'same')
      gradientx = torch.cmul(gradientx, gradientx)
      gradienty = torch.cmul(gradienty, gradienty)
      local gradientsum = torch.add(gradientx, gradienty)
  end
...
trainDataset = tnt.SplitDataset{
    ...
    dataset = tnt.ShuffleDataset{
        dataset = tnt.ListDataset{
            -- list = torch.load(train_split):reshape(1*795):long(),
...
testDataset = tnt.ListDataset{
    -- list = torch.load(test_split):reshape(1*654):long(),
...
-- save model
network:clearState()
torch.save("/home/qc449/project/model.t7",network)
