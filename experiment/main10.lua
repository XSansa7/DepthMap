...
if config.localt == false then
    ...
    TrainSize = N*0.6 -- 1449 * 0.6 * 0.9 = 782
...
-- local xfilter = torch.Tensor(1,3); xfilter[{1,1}] = -1; xfilter[{1,2}] = 0; xfilter[{1,3}] = 1; 
-- local yfilter = torch.Tensor(3,1); yfilter[{1,1}] = -1; yfilter[{2,1}] = 0; yfilter[{3,1}] = 1;
...
--   local sum = 0;
--   local diff = torch.csub(input, target) -- di
--   print(diff:size())
--   local product1 = torch.cmul(diff, diff) -- di^2
--   local product2 = torch.sum(diff, 2) -- sum of di
--   product2 = torch.cmul(product2, product2)
