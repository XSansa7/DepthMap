...
function BuildNetwork2()
    ...
    network2:add(nn.SpatialConvolution(160, 64, 5, 5, 1, 1, 2, 2)
               :learningRate('weight', 10)) --64 74 55
...
function BuildNetwork3()
    ...
    network3:add(nn.SpatialConvolution(64, 64, 5, 5, 1, 1, 2, 2)
                    :learningRate('weight', 10)) --64 147 109
    network3:add(nn.SpatialBatchNormalization(64,1e-3))
    network3:add(nn.ReLU())
    -- network3:add(nn.Dropout(0.3))
    -- --layer 3.3
...
-- save model
network:clearState()
torch.save("/home/qc449/project/model_1219_1.t7",network)
