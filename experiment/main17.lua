...
function BuildPreP2()
    ...
    preprocessing2:add(nn.SpatialConvolution(3, 96, 7, 9, 1, 1, 0, 0)
        :learningRate('weight', 0.0001)
        :learningRate('bias', 0.0002)
        :weightDecay('bias', 0)) -- 150 110
    preprocessing2:add(nn.SpatialBatchNormalization(96))
    preprocessing2:add(nn.ReLU())
...
function BuildNetwork2()
    ...
    network2:add(nn.SpatialConvolution(160, 160, 9, 9, 1, 1, 4, 4)
        :learningRate('weight', 0.0001)
        :learningRate('bias', 0.0002)
        :weightDecay('bias', 0)) --160 74 55
    network2:add(nn.SpatialBatchNormalization(160,1e-3))
    network2:add(nn.ReLU())
    -- network2:add(nn.Dropout(0.3))
    --layer 2.2
    network2:add(nn.SpatialConvolution(160, 64, 5, 5, 1, 1, 2, 2)
        :learningRate('weight', 0.001)
        :learningRate('bias', 0.002)
        :weightDecay('bias', 0)) --64 74 55
    network2:add(nn.SpatialBatchNormalization(64,1e-3))
    network2:add(nn.ReLU())
    -- network2:add(nn.Dropout(0.3))
    --layer 2.3
    network2:add(nn.SpatialConvolution(64, 64, 5, 5, 1, 1, 2, 2)
        :learningRate('weight', 0.001)
        :learningRate('bias', 0.002)
        :weightDecay('bias', 0)) --64 74 55
    network2:add(nn.SpatialBatchNormalization(64,1e-3))
...
-- save model
network:clearState()
torch.save("/home/xj460/project/model_1217_1.t7",network)
