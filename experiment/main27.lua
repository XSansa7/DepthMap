...
if config.localt == false then
    train_image_path = '/home/' .. owener .. '/project/trainImage/'
    train_label_path = '/home/' .. owener .. '/project/trainLabel/'
    train_depth_path = '/home/' .. owener .. '/project/trainDepth/'
...
function rand_crop(input, target)
    ...
    --print(input:size())
    if kind == 1 then
        input = image.crop(input, "c", 236, 312)
    elseif kind == 2 then
        input = image.crop(input, "tl", 236, 312)
    elseif kind == 3 then
        input = image.crop(input, "tr", 236, 312)
    elseif kind == 4 then
        input = image.crop(input, "bl", 236, 312)
    else 
        input = image.crop(input, "br", 236, 312)
    end
...
function BuildPreP2()
    ...
    preprocessing2:add(nn.SpatialConvolution(3, 96, 9, 9, 1, 1, 0, 0)) -- 150 110
...
network:clearState()
network:float()
-- torch.save("/home/" .. owner .. "/project/" ..  version .. ".t7",network)
