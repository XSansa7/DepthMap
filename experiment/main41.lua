...
version = "xjdata_depth15_origin_small"
owener = "qc449"
...
if config.localt == true then
    ...
    train2List = torch.range(1, 3733-1449):long()
    print(train2List:size())
    train2List:add(1449)
    trainList = torch.cat(train2List, trainList, 1)
    
    print(trainList:size())
end

if config.localt == false then
    train_image_path = '/home/' .. owner .. '/project/trainImage/'
    train_depth_path = '/home/' .. owner .. '/project/trainDepth/'
    train_image_path2 = '/scratch/' .. owner .. '/project/trainImage2/'
    train_depth_path2 = '/scratch/' .. owner .. '/project/trainDepth/'
...
function getImage(idx)
    ...
    local path = train_image_path2
    if idx < 1450 then
        path = train_image_path
    end
    local img = transformInput(image.load(path .. file))
...
function getDepth(idx)
    -- print(idx)
    local file = string.format("%04d.t7", idx)
    local path = train_depth_path2
    if idx < 1450 then
        path = train_depth_path
    end
    local depth = torch.load(path .. file):reshape(1,640,480):float()
...
-- criterion2 = nn.SmoothL1Criterion()
-- criterion = nn.MultiCriterion():add(criterion1, 0.8):add(criterion2)
